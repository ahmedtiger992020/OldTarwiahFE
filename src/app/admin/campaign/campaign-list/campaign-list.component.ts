...
import { CampaignVM } from '../../../models/campaignVM.model';
...
export class CampaignListComponent implements OnInit {
  ...
  searchCriteria: CampaignSearchCriteria = new CampaignSearchCriteria(this.filters, this.sortModel, '');
  ...
  searchData(){
    this.campaignService.getCampaigns(this.searchCriteria).subscribe({
      next: (result) => 
        {
          if(!result.isSucess){
  ...
  onUpload() {
    ...
    this.campaignService.importFile(formData).subscribe({
      next: (result) => 
      {
        if(!result.isSucess){
