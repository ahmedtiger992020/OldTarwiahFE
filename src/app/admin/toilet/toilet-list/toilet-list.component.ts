...
@Component({
  ...
  styleUrl: './toilet-list.component.css'
})
export class ToiletListComponent implements OnInit {
  ...
  searchCriteria: ToiletSearchCriteria = new ToiletSearchCriteria(this.filters, this.sortModel, '');
  ...
  searchData(){
    this.toiletService.getToilets(this.searchCriteria).subscribe({
      next: (result) => 
        {
          if(!result.isSucess){
  ...
  onUpload() {
    ...
    this.toiletService.importFile(formData).subscribe({
      next: (result) => 
      {
        if(!result.isSucess){
