...
import { ZamzamLocationVM } from '../../../models/zamzamLocationVM.model';
...
import { ZamzamService } from '../../../services/zamzam.service';
import { MassageService } from '../../../services/massage.service';

@Component({
  selector: 'app-zamzam-list',
  templateUrl: './zamzam-list.component.html',
  styleUrl: './zamzam-list.component.css'
})
export class ZamzamListComponent implements OnInit {
  fileList: File[] = [];
  customFormData = { fileType: 'json' };
  listOfData: ZamzamLocationVM[] = [];
  total = 0;
  loading = true;
  sortValue: string = '';
  sortKey: string = '';
  ...
  searchCriteria: ZamzamSearchCriteria = new ZamzamSearchCriteria(this.filters, this.sortModel, '');

  constructor(private zamzamService: ZamzamService, 
  ...
  searchData(){
    this.zamzamService.getZamZamLocations(this.searchCriteria).subscribe({
      next: (result) => 
        {
          if(!result.isSucess){
  ...
  onUpload() {
    ...
    this.zamzamService.importFile(formData).subscribe({
      next: (result) => 
      {
        if(!result.isSucess){
