...
export class MapComponent implements OnInit, OnChanges {
  ...
  dataLayer: google.maps.Data = new google.maps.Data();
  ToilitsdataLayer: google.maps.Data = new google.maps.Data();
  myMap: google.maps.Map | undefined;
  markers: google.maps.Marker[] = [];
  // polygons: google.maps.Polygon[] = [];
  tempSelectedMarker: any;
  tempMarkers: any[] = [];
  //tempToiletMarkers: any[] = [];
  //tempOtherMarkers: any[] = [];
  //dataLayer:any;
  isSpinning = false;
  ...
  addGeoJsonLayer(): void {
    ...
    this.dataLayer.addListener('click', (event: any) => {
      ...
      this.blockNumberDesc = event.feature!.Fg.NAME;
  ...
  //console.log(event.feature!.Fg);
      ...
      // this.mapSericeService.loadBlockComplain(this.blockNumber)

      //             const infoWindow = new google.maps.InfoWindow({
      //     content: `<div>Block Number: ${this.blockNumber}
      //  <button id="show-detail-btn">Show Detail</button>    </div>`,
      ...
      //         this.markerDetail.emit(this.blockNumber)
      //      //   this.openListTab(); // Call your Angular method
      //       });
      //     }
      //   });
  ...
  async initializeMap() {
    ...
    this.mapSericeService.loadOpenedComplaints(this.MarkersdataLayer)
  }
  ...
  addOrRemoveMapMarker(latLng: any, map: any, isMap: boolean = false) {
    ...
    if (polygon != null) {
      ...
      if (this.Selectedmarker != undefined) {
        ...
        //     if (marker.getPosition()!.equals(new google.maps.LatLng(JSON.parse(this.markerAddedLatLng))) ||
        //     this.tempMarkers.includes(JSON.stringify(marker.getPosition()))) {
        //         marker.setMap(null);
        ...
        //  this.setMapCenterAndZoom(latLng, 13, 0)
  ...
  checkPlockNumber() {
    ...
    this.dataLayer.forEach((feature: any) => {
      ...
      if (isInside) {
        ...
        this.sendSelectedMarkerDetailsToParent();
        this.popup.emit({ showDetailes: false, data: this.SensorDataDetails })
        return;
      }
    });
  }
    ...
    this.toiletService.getAllToilets().subscribe({
      next: (result) => {
        ...
        if (!result.isSucess) {
          ...
          // }
          //this.store.dispatch(setCachedList({ key: 'toilets', list: result.data }));
  ...
  drawMarkers(positions: google.maps.Marker[]) {
    ...
    this.markers!.forEach(marker => {
      ...
      const clusterOptions: MarkerClustererOptions = {
        ...
        maxZoom: 15, // Set the maximum zoom level at which clustering occurs. Beyond this zoom level, markers are not clustered.
  ...
  sendSelectedMarkerDetailsToParent(): void {
    const notification: NotificationData = {
      ...
      blockNumberDesc:this.blockNumberDesc,
      action: ""
    };
